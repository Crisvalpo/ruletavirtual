...
CREATE POLICY "Authenticated insert packages" ON public.game_packages 
    FOR INSERT TO authenticated WITH CHECK (true);

CREATE POLICY "Authenticated update packages" ON public.game_packages 
...
ALTER TABLE public.screen_state
ADD COLUMN idle_speed FLOAT DEFAULT 4.0;
...
-- Function to activate a batch of tickets (by codes array)
DROP FUNCTION IF EXISTS public.activate_ticket_batch(TEXT, TIMESTAMPTZ);
CREATE OR REPLACE FUNCTION public.activate_ticket_batch(p_codes TEXT[])
RETURNS INTEGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  -- Evaluar si es ganador (si el resultado del giro está en sus animales seleccionados)
  ...
  SET status = 'completed',
      spin_result = p_result_index,
      prize_won = CASE WHEN v_is_winner THEN 'PREMIO NIVEL 1' ELSE NULL END,
